...
def load_img_jpg(path_to_img, max_dim):
...
def load_img(path_to_img, max_dim):
    # Read the image file
    img = tf.io.read_file(path_to_img)
    # Decode the image, including the alpha channel if present
    img = tf.image.decode_image(img, channels=4)
    # Convert the image to float32 format
    img = tf.image.convert_image_dtype(img, tf.float32)

    # Separate the image and the alpha channel
    img, alpha = tf.split(img, [3, 1], axis=-1)

    # Resize the image and the alpha channel
    shape = tf.cast(tf.shape(img)[:-1], tf.float32)
    long_dim = max(shape)
    scale = max_dim / long_dim
    new_shape = tf.cast(shape * scale, tf.int32)
    img = tf.image.resize(img, new_shape)
    alpha = tf.image.resize(alpha, new_shape)

    # Concatenate the image and the alpha channel
...
def style_transfer_image(
    ...
    style_img = load_img_jpg(style, max_dim=max_dim)
